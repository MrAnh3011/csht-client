...
import {Pagination, Role} from '../../../model/role.class';
@Injectable()
export class RoleEffects {
  ...
  loadRoles$: Observable<Action> = this.actions$.pipe(
    ...
    map((action: roleAction.GetRoleLoad) => action.pagination),
    mergeMap((page: Pagination) =>
      this.roleService.getRole(page.page - 1, page.pageSize).pipe(
        map(
          (data: {
            totalElements: number;
            content: Role[]
          }) =>
            new roleAction.GetRoleSuccess(data.content, data.totalElements)
        ),
  ...
  editRole$: Observable<Action> = this.actions$.pipe(
    ...
    mergeMap((role: Role) =>
      this.roleService.updateRole(role).pipe(map(() =>
        new roleAction.EditRoleSuccess({id : role.roleId, changes: role})),
        catchError(err => of(new roleAction.DeleteRoleFail(err)))
        ),
    )
  );
